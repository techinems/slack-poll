...
export class Poll {
  private static appendIfMatching(
    optionArray: string[],
    keyword: string,
    appendText: string
  ): string {
    return optionArray[0].toLowerCase() === keyword ||
      optionArray[1].toLowerCase() === keyword
      ? appendText
      : "";
  }
  ...
  static slashCreate(author: string, parameters: string[]): Poll {
    ...
    if (
      optionArray[0].toLowerCase() === "multiple" ||
      optionArray[0].toLowerCase() === "anon"
    ) {
      ...
      mrkdwnValue += this.appendIfMatching(
        optionArray,
        "multiple",
        " *(Multiple Answers)* "
      );
    ...
    const authorBlock: ContextBlock = {
      type: "context",
      elements: [{ type: "mrkdwn", text: `Asked by: ${author}` }]
    };
    ...
    for (let i = start; i < parameters.length; i++) {
      ...
      const button: Button = {
        type: "button",
        value: " ",
        text: this.buildTextElem(parameters[i])
      };
    ...
    if (
      optionArray[0].toLowerCase() === "anon" ||
      optionArray[1].toLowerCase() === "anon"
    ) {
      selection.options!.push(
        this.buildSelectOption("Collect Results", "collect")
      );
    }
    actionBlockCount++;
  ...
  public getAuthor(): string {
    return ((this.message[1] as ContextBlock)
      .elements[0] as PlainTextElement).text.replace("Asked by: ", "");
  }
  ...
  private getVotesAndUserIndex(
    button: Button,
    userId: string
  ): { votes: string[]; userIdIndex: number } {
  ...
  public lockPoll(): void {
    this.message = this.message
      .slice(0, 2)
      .concat(this.message.slice(this.getDividerId() - 1));
  ...
  public collectResults(): KnownBlock[] {
    ...
    return [
      Poll.buildSectionBlock(
        `${this.getTitleFromMsg()} *RESULTS (Confidential do not distribute)*`
      )
    ].concat(results);
  }
  ...
  private processButtons(
    loopEnd: number,
    buttonCallback: (b: Button) => boolean
  ): void {
  ...
  private resultGeneratorHelper(overrideAnon: boolean): SectionBlock[] {
    ...
    for (const key in votes) {
      ...
      const names =
        !this.anonymous || overrideAnon
          ? users.map((k: string) => `<@${k}>`).join(",")
          : "~HIDDEN~";
      responseSections.push(
        Poll.buildSectionBlock(`*${users.length}* ${key} » ${names}`)
      );
    }
  ...
  private generateVoteResults(): void {
    ...
    this.message = this.message
      .slice(0, this.getDividerId() + 1)
      .concat(this.resultGeneratorHelper(false));
  }
